...
function TabItem({ item, isActive }: { item: typeof navItems[0]; isActive: boolean }) {
    ...
    return (
        <Link
            href={item.href}
            onClick={handleTap}
            className="relative flex flex-col items-center justify-center w-[56px] h-[46px] rounded-[18px] touch-manipulation"
        >
            {/* Active glass pill */}
            {isActive && (
                <motion.div
            ...
            <motion.span
                ...
                className={`relative z-10 text-[9px] font-semibold leading-none mt-[1px] ${isActive ? "text-blue-500" : "text-gray-400/50"
                    }`}
            >
                {item.label}
            </motion.span>
    ...
    if (variant === "bottom") {
        ...
        return (
            <div className="fixed bottom-0 left-0 right-0 z-[100] md:hidden pointer-events-none">
                ...
                <div className="flex justify-center px-6 pb-12 safe-bottom pointer-events-auto">
                    <nav
                        className="flex items-center justify-around w-full max-w-[280px] px-1 py-[5px] rounded-full"
                        style={{
                            background:
